...
Abdul-Rahman in Hailes predlagata porazdeljeni model zaupanja, ki temelji na izračunu zaupanja iz verig priporočil med agenti \cite{abdul1998distributed}.
Vsak agent hrani diskretne vrednosti zaupanja na lestvici od $-1$ do $4$, kjer $-1$ pomeni popolno nezaupanje, $0$ nevednost (agent si ne upa podati ocene), vrednosti $1$ do $3$ označujejo naraščajoče stopnje zaupanja, $4$ pa predstavlja popolno zaupanje.
Ločita med:
\begin{itemize}
  \item \emph{vrednostjo priporočiteljevega zaupanja} $\mbox{tv}(R_i)$, ki opisuje, koliko agent zaupa $i$-temu priporočitelju na poti in
...
\end{equation}
Ker lahko agent za isto ciljno entiteto $T$ prejme več priporočil po različnih poteh, naj bo $r$ število teh poti, označenih z $i = 1, \dots, r$. Vse delne ocene zaupanja $\mbox{tv}_1(T), \dots, \mbox{tv}_r(T)$ se povprečijo, tako da se končna priporočilna ocena zaupanja v entiteto $T$ izračuna kot
