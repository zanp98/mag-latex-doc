...
Ločita med:
\begin{itemize}
  \item \emph{vrednostjo priporočiteljevega zaupanja} $tv(R_i)$, ki opisuje, koliko agent zaupa $i$-temu priporočitelju na poti in
  \item \emph{priporočeno vrednostjo zaupanja} $rtv(T)$, ki jo o ciljni entiteti $T$ poda zadnji priporočitelj v verigi.
\end{itemize}
...
Zaupanje v ciljno entiteto $T$ po poti $p$ označimo z $tv_p(T)$.
Avtorja predlagata naslednjo formulo za izračun zaupanja po poti $p$ \cite{abdul1998distributed}:

\begin{equation}
  tv_p(T) = \left( \prod_{i=1}^{n} \frac{tv(R_i)}{4} \right) \cdot rtv(T)
  \label{eq:abdulrahman-path}
\end{equation}

Ker lahko agent za isto ciljno entiteto $T$ prejme več priporočil po različnih poteh, naj bo $p$ število teh poti, označenih z $i = 1, \dots, p$. Abdul-Rahman in Hailes predlagata, da se vse delne ocene zaupanja $tv_1(T), \dots, tv_p(T)$ povprečijo, tako da se končna priporočilna ocena zaupanja v entiteto $T$ izračuna kot
$tv(T) = \frac{1}{p} \sum_{i=1}^{p} tv_i(T)$ \cite{abdul1998distributed}.
...
\begin{itemize}
  ...
  \item $Cr(v,t) \in [0,1]$ je faktor uravnoteženja zaupanja, ki zmanjšuje tveganje neverodostojnih povratnih informacij iz vozlišča $v$.
\end{itemize}
...
V skladu z definicijo metrike PeerTrust privzamemo, da za vse $v \in P, v \neq u$ in dani $t$ velja
$0 \leq S(u,v,t) \leq I(u,v,t)$ ter $0 \leq Cr(v,t) \leq 1$, poleg tega pa ima akter $u$ vsaj eno interakcijo, tj.\ $\sum_{v \in P, v \neq u} I(u,v,t) > 0$.

Ker je $S(u,v,t) \ge 0$ in $Cr(v,t) \in [0,1]$, sledi
$0 \leq S(u,v,t)\,Cr(v,t) \leq S(u,v,t)$.
Zaradi $S(u,v,t) \leq I(u,v,t)$ dobimo
$0 \leq S(u,v,t)\,Cr(v,t) \leq I(u,v,t)$
za vse $v \in P, v \neq u$.
