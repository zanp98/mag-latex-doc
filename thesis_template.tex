...
te metrike zapisane v JSON datoteki \texttt{state/telemetry\_metrics.json}:
...
status transporterja. Spremeni se torej le operativni del – telemetrija.

V realnem sistemu tak zapis ne bi bil lokalna JSON datoteka, temveč
projekcija zunanjih podatkov:
\begin{itemize}
  \item meritve iz IoT senzorjev (temperature, šok, vlaga) v hladni verigi,
  \item zapisi iz transportnega informacijskega sistema (TMS) o zamudah,
  \item alarmi iz nadzornih sistemov ali sistemov za kakovost.
\end{itemize}
Vsak orakelj bi bil konfiguriran z lastnimi adapterji do takih virov (npr.\
API-ji, sporočilni sistemi, podatkovna jezera). Nekaterim orakljem (npr.\
telemetrijskemu) so ti podatki ključni, drugi (npr.\ VC-centrični) jih
namerno ignorirajo ali jim dajejo zelo majhno težo. V prototipu te
razlike modeliramo z različnimi algoritmi orakljev A, B in C nad istim
simuliranim telemetrijskim virom.

\subsubsection*{Novi izračuni orakljev}

Na podlagi spremenjenih podatkov oraklji ponovno izvedejo svoje algoritme
in pripravijo nova podpisana poročila. Rezultati so:
...
\begin{itemize}
  ...
  \item \textbf{Zastavice} Oraklja A in C nastavljata zastavico
        ...
        ravni.
  \item \textbf{Politike in poverilnice} Vsa tri poročila uporabljajo
        enak \texttt{credential\_hash}, kar pomeni, da se še vedno sklicujejo
        na isto veljavno preverljivo poverilnico. Agregator pri tem preveri,
        da poverilnica v lokalni shrambi ni označena kot preklicana. To
        pomeni, da je sprememba ocene iz \emph{PASS} (scenarij 1) v
        \emph{FAIL} (scenarij 2) posledica izključno poslabšanja
        operativnih metrik in zunanjih podatkov, ne pa spremembe
        formalnih dokazil.
\end{itemize}
...
(\emph{FAIL}) z agregirano številčno oceno zaupanja približno
$6267/10000$ in z zastavico \texttt{0x02}, ki označuje nizko oceno.
Primerjava scenarijev~\ref{subsec:scenarij-1} in~\ref{subsec:scenarij-2}
jasno pokaže, da sistem ni statičen: isti subjekt se ob poslabšanju
telemetrije in drugih zunanjih podatkov premakne iz zaupanja vrednega v
nezaupanja vreden, kljub temu da njegovo preverljivo digitalno potrdilo
