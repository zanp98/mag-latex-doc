...
V informacijskih infrastrukturah zaupanje ne obravnavamo le kot intuitiven občutek, temveč kot količino, ki jo poskušamo formalno zapisati, izračunati in uporabiti kot vhod v algoritme za odločanje. Tipičen cilj modeliranja zaupanja je, da iz opazovanj in dokazov (preteklih transakcij, priporočil, kriptografskih dokazil) izpeljemo numerično oceno zaupanja $T(u,v) \in [0,1]$ med subjektom $u$ in subjektom $v$, ki jo nato uporabimo pri avtorizaciji, izbiri partnerja ali odkrivanju zlonamernih udeležencev \cite{cho2015survey}. Pri tem moramo odločiti, kako bomo zaupanje predstavili (kot številčno oceno, interval, porazdelitev ali vektor lastnosti), kako bomo zbirali in tehtali dokaze ter kako bomo iz ocen izpeljali končne binarne odločitve.

Yan in Holtmanns podata taksonomijo modelov zaupanja, kjer modele razvrstita glede na način modeliranja (lingvistični, grafični, matematični), glede na število obravnavanih lastnosti (eno- ali večlastnostni modeli), način izražanja zaupanja (binarne, diskretne ali kontinuirne ocene) in dimenzionalnost izraza zaupanja.\cite{yan2008trustmodeling} Ta razvrstitev pokaže, da je že na konceptualni ravni treba izbrati, ali bomo zaupanje obravnavali kot enodimenzionalen “score” ali kot večkomponentni opis (npr.\ kompetentnost, integriteta, dobrohotnost), ter ali bomo negotovost izrazili eksplicitno ali pa jo skrijemo v samo številčno vrednost zaupanja.

Cho s sodelavci povzema, da sodobni modeli zaupanja načeloma delijo nekaj skupnih lastnosti: zaupanje je usmerjeno (od trustorja k trusteeju), je odvisno od konteksta, je dinamično in ga je mogoče meriti ter posodabljati skozi čas.\cite{cho2015survey} Avtorji predlagajo ogrodje, v katerem modeliranje zaupanja poteka v treh korakih: (i) zbiranje evidenc o obnašanju subjektov in njihovem okolju, (ii) preslikava teh evidenc v formalno predstavitev zaupanja z ustrezno matematično funkcijo ter (iii) odločanje, kjer sistem uporabi oceno zaupanja in politiko (npr.\ pragove ali pravila), da sprejme ali zavrne sodelovanje. V tem okviru se razlikujejo predvsem izbira matematičnega formalizma (verjetnostne porazdelitve, teorija dokazov, fuzzy logika, grafični modeli) ter način, kako sistem združuje več virov informacij in upravlja negotovost.

V tej nalogi se umeščamo v družino statističnih, na dokazih utemeljenih modelov zaupanja, kot jih identificira Cho in sodelavci.\cite{cho2015survey} Zaupanje v izpolnjevanje posameznih kriterijev modeliramo z beta porazdelitvami, ki se posodabljajo glede na opažene dogodke, podobno kot v beta reputacijskih pristopih.\cite{josang2002beta} Pri tem upoštevamo tudi količino zbranih dokazov in časovni vidik: z amortizacijskim faktorjem tipa EWMA dajemo večjo težo novejšim opazovanjem, končne ocene pa kombiniramo z determinističnimi pravili v ontološkem modelu in jih pretvorimo v binarne odločitve skladno s politikami zaupanja.
...
Verjetnostni del iste kriterije pretvori v številsko verjetnostno oceno v intervalu $[0,1]$, ki jo orakelj kasneje pretvori v osnovne točke med 0 in 10000. Za vsak kriterij se vodi aposterior prazdelitve beta $B(\alpha, \beta)$, ki modelira zaupanje za izpolnitev tega kriterija.
