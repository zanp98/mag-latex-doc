...
% \subsection{Matematični pristopi k zaupanju}

% Gambetta zaupanje definira kot verjetnost, da bo drugi akter izvedel določeno dejanje v kontekstu, ki vpliva na njegovo lastno ravnanje \cite{gambetta1988trust}. Iz interpretacije zaupanja kot verjetnosti so se razvili številni matematični modeli, ki poskušajo iz pozitivnih in negativnih izkušenj izpeljati napoved obnašanja. Preglede teh pristopov podajajo Yan in Holtmanns v članku \textit{Trust modeling and management: from social trust to digital trust} \cite{yan2008trust}, Wang in Singh v članku \textit{Evidence-based trust: A mathematical model geared for multiagent systems} \cite{wang2010evidence} ter Kochovski in sodelavci  v članku \textit{Trust management in a blockchain based fog computing platform with trustless smart oracles} \cite{kochovski2019trust}. Ker je pristopov veliko, v nadaljevanju povzemamo le tiste, ki so najbolj pomembni za zasnovo sistema v tej nalogi.

% Jøsang in Ismail v članku \emph{The Beta Reputation System} predstavita pristop, ki zaupanje razume kot statistični model nad binarnimi izidi transakcij (uspeh ali neuspeh) \cite{josang2002beta}. Sistem temelji na verjetnostni porazdelitvi beta, ki opisuje verjetnost, da se bo subjekt v prihodnje vedel pošteno. Parametra  porazdelitve beta sta neposredno povezana s količino zbranih povratnih informacij: $r$ predstavlja vsoto pozitivnih izkušenj, $s$ pa vsoto negativnih izkušenj, pričakovana vrednost porazdelitve $E[p] = (r+1)/(r+s+2)$ pa služi kot ocena zaupanja. Pomemben del modela je zmanjšanje teže povratnih informacij glede na zanesljivost ocenjevalca, pri čemer se avtorja opirata na Jøsangovo subjektivno logiko, ki zaupanje opiše s trojico $(b,d,u)$, kjer $b$ predstavlja prepričanje, $d$ dvom in $u$ negotovost \cite{josang1999algebra}. Poleg tega uvedeta še mehanizem postopnega pozabljanja starih povratnih informacij s faktorjem $\lambda$, ki omogoča, da ima novejše vedenje večji vpliv na oceno zaupanja. Avtorja predpostavljata zbiranje povratnih informacij, kjer uporabniki podajajo preproste ocene. Te se pretvorijo v parametre porazdelitve beta, kar poveže matematično podlago s praktično rabo v spletnih trgovinah.
...
Naj bo $p$ indeks konkretne poti priporočil, $n$ število priporočiteljev na tej poti, $R_1, R_2, \dots, R_n$ pa zaporedni priporočitelji na poti od izvornega priporočitelja do tistega, ki priporočilo zahteva.
...
Ker lahko agent za isto ciljno entiteto $T$ prejme več priporočil po različnih poteh, naj bo $p$ število teh poti, označenih z $i = 1, \dots, p$. Abdul-Rahman in Hailes predlagata, da se vse delne ocene zaupanja $\mbox{tv}_1(T), \dots, \mbox{tv}_p(T)$ povprečijo, tako da se končna priporočilna ocena zaupanja v entiteto $T$ izračuna kot
$\mbox{tv}(T) = \frac{1}{p} \sum_{i=1}^{p} \mbox{tv}_i(T)$ \cite{abdul1998distributed}.
...
\end{equation}
kjer je v implementaciji uporabljena utežni faktor $\textit{obs\_scale} = 9$. Za »popolno« izpolnjen pogoj ($s_0 \approx 1$) dobimo pričakovano vrednost porazdelitve beta $\mathbb{E}[X] \approx 0{,}91$, kar predstavlja visoko zaupanje.
...
\begin{equation}
    \alpha' = (1 - \gamma) \cdot \alpha + \gamma \cdot (1 + s \cdot \textit{obs\_scale}),
\end{equation}
\begin{equation}
    \beta' = (1 - \gamma) \cdot \beta + \gamma \cdot (1 + (1 - s) \cdot \textit{obs\_scale}).
\end{equation}
...
Za posameznega akterja se končna verjetnostna ocena $\textit{prob\_score}$ izračuna kot uteženo povprečje srednjih vrednosti beta aposteriorjev ($\frac{\alpha}{\alpha+\beta}$) vseh aktivnih kriterijev. Uteži $w_i$, zapisane v pravilih ocenjevalca (v našem primeru oraklja), so normalizirane tako, da velja $\sum w_i = 1$.

Kombinirana odločitev združuje trda pravila z mehko oceno. Če orakelj uporablja prag $\textit{prob\_threshold}$, je odločitev:
\[
    \textit{combined} = \textit{deterministic} \;\lor\; \left(\textit{prob\_score} \geq \textit{prob\_threshold}\right).
\]
...
Ključna značilnost je, da brez ustreznih preverljivih poverilnic odločitev nagiba k \emph{FAIL}, tudi če so druga operativna merila razmeroma dobra. To odraža močan poudarek na formalnih dokazih in skladnosti.
...
Za proizvajalca BioPharm sta zabeležena \texttt{tempDev} $= 0{,}08$ in \texttt{GDPVC=true}. Oba pogoja sta deterministično izpolnjena (\emph{PASS}), zaželenost je visoka in oba kriterija dobita srednjo vrednost posteriorja beta približno $0{,}91$. Verjetnostna ocena je zato:
\[
...
Za proizvajalca CanaryLabs sta zabeležena \texttt{tempDev} $= 0{,}12$ in \texttt{GDPVC=false}. Deterministični del zato vrne \emph{FAIL}. Ker je temperaturno odstopanje prevelike in ni poverilnice o skladnosti, je tudi verjetnostna ocena nizka ($\textit{prob\_score} \ll 0{,}7$). Kombinirana odločitev je:
\[
...
\[
  \text{odločitev} =
  \begin{cases}
    \text{true}, & \text{če vsaj dva od treh orakljev poročata \text{true}},\\
    \text{false}, & \text{sicer}.
  \end{cases}
\]
...
\begin{itemize}
  ...
  \item \textbf{Zastavice} Oraklja A in C nastavljata zastavico
        ...
        ravni.
  \item \textbf{Politike in poverilnice} Vsa tri poročila uporabljajo enako zgoščeno vrednost poverilnic \texttt{credential\_hash}, kar pomeni, da se še vedno sklicujejo na isto veljavno preverljivo poverilnico. Agregator pri tem preveri, da poverilnica ni označena kot preklicana. To pomeni, da je sprememba ocene iz \emph{PASS} (scenarij 1) v \emph{FAIL} (scenarij 2) posledica izključno poslabšanja operativnih meril in zunanjih podatkov, ne pa spremembe formalnih dokazil.
\end{itemize}
