...
Sistem sestavlja več vsebnikov Docker, ki so povezani v skupno virtualno omrežje in delujejo z orodjem \texttt{Docker Compose}. V nadaljevanju so opisani posamezni vsebniki in njihove vloge.
...
\paragraph{Baza znanja} V vsebniku Fuseki je naložena domenska ontologija. Ta je dostopna preko SPARQL končne točke, ki jo oraklji uporabljajo za pridobivanje semantičnih lastnosti akterjev. V eksperimentalnem okolju uporabljamo ontologijo, ki ga vsi oraklji delijo. Ontologija je predstavljena v prilogi \ref{app:trust-ontology}.

\paragraph{Oraklji} Za evalvacijo smo pripravili tri neodvisne oraklje, ki izvajajo različne politike izračuna zaupanja. Vsak orakelj teče v svojem vsebniku Docker in je konfiguriran z ločeno datoteko, ki določa pravila ocenjevalca, nabor vhodnih podatkov ter težo posameznih kriterijev. Oraklji so implementirani v Pythonu in uporabljajo knjižnice za dostop do verige blokov (web3.py), baze znanja (rdflib, SPARQLWrapper) ter za kriptografske operacije.

\paragraph{Agregator}, ki zbira poročila posameznih orakljev, preverja njihove podpise in objavi končno odločitev v pametni pogodbi na verigi.
...
Kontejnerji si delijo notranje Docker omrežje, zato med seboj komunicirajo preko stabilnih DNS imen (npr.\ \texttt{fuseki:3030}, \texttt{ethereum-node:8545}).

\paragraph{Konfiguracija orakljev}
Vsi oraklji uporabljajo isti paket knjižnic za dostop do baze znanja, verige blokov in za kriptografske operacije, razlikujejo pa se v konfiguraciji in politikah izračuna zaupanja. Vsak orakelj ima svojo konfiguracijsko datoteko, v kateri so zapisani:
\begin{itemize}
    \item referenca na \emph{skupno bazo znanja} (isti Fuseki dataset),
    \item nabor \emph{poverilnic in dokazov} (VC), ki jih pri izračunu obravnava kot vhod,
    \item \emph{pravila in uteži} za posamezne kriterije zaupanja.
\end{itemize}

S tem modeliramo, da v omrežju sodelujejo neodvisni evalvatorji, ki uporabljajo različne poglede na iste podatke. Na primer, v tipični konfiguraciji:
\begin{itemize}
    \item prvi orakelj poudarja regulatorni vidik in daje večjo težo veljavnim certifikatom in regulatornim odločbam,
    \item drugi orakelj se osredotoča na operativno zanesljivost, kjer imajo večjo težo incidenti pri transportu, temperaturna odstopanja in zanesljivost IoT senzorjev,
    \item tretji orakelj kombinira zgodovino sodelovanja med akterji in pretekle odločitve o zaupanju ter tako deluje bolj reputacijsko usmerjeno.
\end{itemize}
...
\paragraph{Povzetek konfiguracije.}
Tabela~\ref{tab:eksperimentalna-konfiguracija} povzema ključne parametre eksperimentalne postavitve, ki jih v nadaljevanju uporabljamo v obeh scenarijih.

\begin{table}[H]
    \centering
    \caption{Povzetek eksperimentalne konfiguracije sistema.}
    \label{tab:eksperimentalna-konfiguracija}
    \begin{tabular}{ll}
        \hline
        Parameter & Vrednost \\
        \hline
        Okolje verige blokov & lokalni testnet (Docker kontejner) \\
        Naslov pametne pogodbe \texttt{TrustGraph.sol} & \texttt{0x\ldots{}} (primer v prilogi) \\
        Baza znanja & Apache Jena Fuseki, en dataset \\
        Število orakljev & 3 (različne politike zaupanja) \\
        Agregacija poročil & prag soglasja (npr.\ večina) \\
        Odjemalski vmesnik & Python skripta / ukazna vrstica \\
        \hline
    \end{tabular}
\end{table}
...
V poglavju~\ref{sec:trust-management-distributed-systems} smo predstavili različne pristope k upravljanju zaupanja v razpršenih sistemih: dokazne modele, priporočilne modele, porazdeljeno ocenjevanje ter dinamično posodabljanje zaupanja. Ti koncepti tvorijo teoretično podlago za zasnovo naših orakljev. V nadaljevanju pokažemo, kako vsak orakelj uteleša drugačen pogled na zaupanje:
\begin{itemize}
    \item Pfizerjev orakelj uporablja hibridni pristop, ki kombinira dokazne elemente (licence, certifikati) z dinamično metriko, podobno priporočilnim in porazdeljenim modelom.
    \item Orakelj Moderna je izrazito \emph{dokazni} – odloča skoraj izključno na podlagi verifiable credentials, podobno kot hierarhični in PGP-sistemi iz podpoglavja o dokaznih modelih.
    \item Orakelj DHL je \emph{telemetrijsko usmerjen} in se osredotoča na dinamično posodabljanje zaupanja preko operativnih metrik, kar je skladno z modeli, ki večjo težo dajejo nedavnim interakcijam.
\end{itemize}
...
\begin{enumerate}
    ...
    \item probabilistični del (številčna ocena),
    \item kombinirano odločitev.
\end{enumerate}

\paragraph{Deterministični del}
Deterministični del temelji na politikah, ki so zapisane v konfiguracijskih datotekah (\texttt{policy.json}). V njih so za posamezne tipe subjektov (proizvajalec, distributer, transporter) definirani kriteriji in pragovi, npr.\:
\begin{itemize}
...
\end{itemize}
Če subjekt za vse zahtevane kriterije zadošča pogojem, deterministični del vrne odločitev \emph{PASS}, sicer \emph{FAIL}. Ta del je najbližje \emph{dokaznim modelom zaupanja}, kjer se odločitve sprejemajo na podlagi preverljivih dokazov in trdih pragov.

\paragraph{Probabilistični del}
Probabilistični del pretvori iste kriterije v numerični score v intervalu $[0,1]$, ki ga orakelj kasneje pretvori v basis points (0--10000). Za vsak kriterij se vodi Beta posterior, ki se posodablja po principu eksponentno uteženega drsečega povprečja (EWMA). S tem približamo obnašanje modelom iz podpoglavij o priporočilnih modelih in dinamičnem posodabljanju zaupanja.
...
Ob novih opazovanjih se posterior posodablja z EWMA faktorjem $\gamma \approx 0{,}2$, kar zagotavlja, da nove informacije hitreje vplivajo na score, stare pa postopoma izgubijo težo. To je konceptualno podobno formuli za dinamično posodabljanje zaupanja v podpoglavju o amortizacijskih faktorjih, kjer nove interakcije s časom prevladajo nad starimi.

\paragraph{Kombinirana odločitev}
Za posamezen subjekt se probabilistični score izračuna kot uteženo povprečje srednjih vrednosti Beta posteriorjev aktivnih kriterijev. Uteži so zapisane v polju \texttt{\_weights} v politiki in se normalizirajo tako, da zadošča:
\[
    \sum_i w_i = 1, \qquad w_i \geq 0.
\]

Vsak orakelj uporablja svoj prag \textit{prob\_threshold}, ki ga izražamo na lestvici 0--1 (v implementaciji pa v basis points). Kombinirana odločitev ima obliko:
\[
    \textit{combined} = \textit{deterministic} \;\text{OR}\; \left(\textit{prob\_score} \geq \textit{prob\_threshold}\right),
\]
kjer je \textit{deterministic} deterministična odločitev (\emph{PASS}/\emph{FAIL}), \textit{prob\_score} pa izračunani probabilistični score. Na ta način se trda pragovna pravila (dokazni pogled) kombinirajo z mehko, numerično oceno, ki je bližje priporočilnim in porazdeljenim modelom zaupanja.
...
\subsubsection*{Pfizer orakelj (hibridni pogled)}

Pfizerjev orakelj uporablja hibridni pogled, ki združuje regulatorne kriterije, kakovost delovanja in informacije iz verifiable credentials. S tem v praksi kombinira dokazni model (digitalna dokazila, licence) z dinamičnim ocenjevanjem in uteženjem kriterijev preko Beta posteriorjev. Njegov prag za probabilistični score je nastavljen na:
\[
    \textit{prob\_threshold}^{(\text{Pfizer})} = 0{,}75.
\]
...
Za distributerja MedLogix v trenutni ontologiji velja \texttt{license=true} in \texttt{GDPVC=true}. Deterministični del zato vrne \emph{PASS}. Oba kriterija imata visoko zaželenost, zato se Beta posteriorja inicializirata z mean $\approx 0{,}91$. Po normalizaciji uteži (približno $0{,}44$ in $0{,}56$) dobimo probabilistični score:
\[
    \textit{prob\_score} \approx 0{,}91 \geq 0{,}75.
\]
Kombinirana odločitev je:
\[
    \textit{combined}^{(\text{Pfizer}, \text{MedLogix})} = \textit{PASS}.
\]

\paragraph{Primer: SkyFreight (transporter).}
Za transporterja SkyFreight sta zabeleženi vrednosti \texttt{punctuality} $= 0{,}90$ in \texttt{tempViolationRate} $= 0{,}08$. Deterministični pogoji niso izpolnjeni, ker $0{,}90 < 0{,}99$ in $0{,}08 > 0{,}05$, zato deterministični del vrne \emph{FAIL}. Tudi normalizirana zaželenost $s_0$ za oba kriterija je nizka, kar vodi v nizek Beta mean in probabilistični score $\textit{prob\_score} < 0{,}75$. Končna odločitev oraklja je:
\[
    \textit{combined}^{(\text{Pfizer}, \text{SkyFreight})} = \textit{FAIL}.
\]

\subsubsection*{Moderna orakelj (VC-centričen pogled)}

Modernin orakelj je zasnovan tako, da v ospredje postavlja verifiable credentials. Konceptualno je najbližje dokaznim modelom zaupanja iz poglavja~\ref{sec:trust-management-distributed-systems}, kjer imata veriga potrdil in kriptografska dokazila ključno vlogo pri inicializaciji zaupanja. Njegov prag za probabilistični score je strožji:
\[
    \textit{prob\_threshold}^{(\text{Moderna})} = 0{,}8.
\]
...
Ključna značilnost je, da brez ustreznih VC odločitev nagiba k \emph{FAIL}, tudi če so druge operativne metrike razmeroma dobre, kar odraža močan poudarek na formalnih dokazih in skladnosti.

\paragraph{Primer: MedLogix (distributer).}
Ker ima MedLogix veljavne VC in licenco, deterministični del vrne \emph{PASS}. Za oba kriterija dobimo Beta mean $\approx 0{,}91$, uteži so po normalizaciji $0{,}67$ in $0{,}33$, zato:
\[
    \textit{prob\_score} \approx 0{,}91 \geq 0{,}8,
\]
in kombinirana odločitev ostane:
\[
    \textit{combined}^{(\text{Moderna}, \text{MedLogix})} = \textit{PASS}.
\]

\paragraph{Primer: HealthChain (transporter).}
Za transporterja HealthChain je v bazi znanja zabeležen \texttt{recallRate} $= 0{,}015$, kar deterministično zadošča pogoju \texttt{hasRecallRate} $\leq 0{,}02$ in daje \emph{PASS}. Pri probabilističnem delu pa je zaželenost pri vrednosti blizu zgornje meje nizka (npr.\ $s_0 \approx 0{,}25$), kar vodi v Beta mean približno $0{,}30$ in:
\[
    \textit{prob\_score} \approx 0{,}30 < 0{,}8.
\]
Kombinirana odločitev je zato:
\[
    \textit{combined}^{(\text{Moderna}, \text{HealthChain})} = \textit{FAIL},
\]
kar ilustrira strožji, VC-centričen pristop: deterministični \emph{PASS} sam po sebi ni zadosten brez visoke probabilistične ocene.

\subsubsection*{DHL orakelj (telemetrija-prvi pogled)}

DHL-jev orakelj uporablja telemetrijo transporta (časovne serije, agregirane metrike) kot primarni vir informacij. Konceptualno se približa porazdeljenim in dinamičnim modelom zaupanja iz poglavja~\ref{sec:trust-management-distributed-systems}, kjer se zaupanje gradi na osnovi kumulativnih izkušenj in se sproti prilagaja novim dogodkom. Njegov prag za probabilistični score je:
\[
    \textit{prob\_threshold}^{(\text{DHL})} = 0{,}7.
\]
...
Za proizvajalca BioPharm sta zabeležena \texttt{tempDev} $= 0{,}08$ in \texttt{GDPVC=true}. Oba pogoja sta deterministično izpolnjena (\emph{PASS}), zaželenost je visoka in oba kriterija dobita Beta mean $\approx 0{,}91$. Probabilistični score je zato:
\[
    \textit{prob\_score} \approx 0{,}91 \geq 0{,}7,
\]
kombinirana odločitev pa:
\[
    \textit{combined}^{(\text{DHL}, \text{BioPharm})} = \textit{PASS}.
\]

\paragraph{Primer: CanaryLabs (proizvajalec).}
Za proizvajalca CanaryLabs sta zabeležena \texttt{tempDev} $= 0{,}12$ in \texttt{GDPVC=false}. Deterministični del zato vrne \emph{FAIL}. Ker sta temperaturna odstopanja prevelika in ni VC o skladnosti, je tudi probabilistični score nizek ($\textit{prob\_score} \ll 0{,}7$). Kombinirana odločitev je:
\[
    \textit{combined}^{(\text{DHL}, \text{CanaryLabs})} = \textit{FAIL}.
\]

\medskip

Takšna predstavitev treh orakljev neposredno gradi na modelih iz poglavja~\ref{sec:trust-management-distributed-systems}: Pfizerjev orakelj uteleša hibridni, semantično in dinamično podprt pristop, Moderna orakelj predstavlja strogo dokazno-kripto\-grafski pogled, DHL orakelj pa dinamično, na meritvah temelječe ocenjevanje. V scenarijih, ki sledijo, bomo za izbranega subjekta pokazali, kako različni algoritmi privedejo do različnih, a razložljivih odločitev o zaupanju.

\subsection{Scenarij 1: Ocena zaupanja akterja}

V prvem scenariju si bomo pogledali, kako v sistemu pridobimo nove vrednosti zaupanja za določenega akterja. Predpostavimo, da želimo posodobiti oceno zaupanja za transportno podjetje \texttt{DHL}, ki sodeluje v naši dobavni verigi.
