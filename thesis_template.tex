...
Farmacevtska dobavna veriga povezuje proizvajalce učinkovin in končnih izdelkov, distributerje, transporterje in lekarne v zapleten ekosistem, v katerem se prepletajo regulativne zahteve, poslovni interesi in kritična odgovornost do pacientov. Napačne odločitve o tem, komu zaupati pri proizvodnji, skladiščenju ali transportu, lahko vodijo v ogroženo kakovost zdravil, prekinitve preskrbe ali celo ogrožanje zdravja. V zadnjih letih so regulatorji in industrija veliko pozornosti namenili sledljivosti serij, preprečevanju ponarejenih zdravil in nadzoru nad dobavitelji, vendar mehanizmi za upravljanje zaupanja ostajajo razpršeni in pogosto slabo formalizirani.

V praksi se zaupanje med akterji dobavne verige še vedno pogosto vzpostavlja na podlagi dvostranskih pogodb, ročnih revizij in zaprtih evidenc. Tak pristop otežuje ponovljivost odločitev in uporabo podatkov v avtomatiziranih procesih. Regulatorji nimajo enotnega, strojno preverljivega pogleda na to, kako so ocene zaupanja nastale, posamezni akterji pa težko dokazujejo svojo skladnost in kakovost prek različnih tržnih in nacionalnih meja. Ob tem se povečuje količina podatkov, ki bi lahko vplivali na oceno zaupanja (telemetrija transporta, digitalna dokazila, revizijska poročila), vendar ti podatki niso povezani v enoten, transparenten mehanizem odločanja.

Vzporedno z razvojem regulative so nastale tehnologije, ki omogočajo bolj strukturirano obravnavo zaupanja. Semantične tehnologije in ontologije omogočajo eksplicitno modeliranje vlog, relacij in dokazov v dobavni verigi. Verige blokov pa ponujajo nespremenljivo plast za zapis odločitev in revizijsko sled. Preverljive poverilnice (angl. verifiable credentials) prinašajo standardiziran način za izdajanje in preverjanje digitalnih dokazil, decentralizirana omrežja orakljev (angl. decentralized oracle network) pa omogočajo, da se izračuni zaupanja izvajajo zunaj verige, rezultati pa se v preverljivi obliki zapišejo na verigo.

Cilj magistrske naloge je raziskati, kako se lahko ti gradniki povežejo v enoten mehanizem za izračun in objavo odločitev o zaupanju v farmacevtski dobavni verigi. Osrednje raziskovalno vprašanje se glasi, ali lahko kombinacija ontologije, preverljivih poverilnic, decentraliziranega omrežja orakljev in pametne pogodbe na verigi blokov omogoči razložljivo in kriptografsko preverljivo upravljanje zaupanja med akterji dobavne verige.

Na tej podlagi zasledujemo več konkretnih ciljev:
\begin{itemize}
  \item oblikovati konceptualni model zaupanja v farmacevtski dobavni verigi, ki povezuje tipe akterjev, njihove lastnosti in vire dokazov o njihovem delovanju,
  \item razviti ontologijo v jeziku OWL, ki ta model formalizira in omogoča poizvedovanje ter osnovno inferenco nad podatki,
  \item zasnovati arhitekturo sistema, ki ontologijo poveže s pametno pogodbo in decentraliziranim omrežjem orakljev na verigi blokov,
  \item implementirati prototipni sistem na testnem omrežju Ethereum z več oraklji, ki uporabljajo različne pristope k računanju ocen zaupanja,
  \item pripraviti scenarije uporabe, s katerimi prikažemo delovanje sistema pri ocenjevanju konkretnih akterjev in dinamičnem prilagajanju ocene zaupanja,
  \item izvesti varnostno analizo pametne pogodbe in omrežja orakljev ter opredeliti ključne korake na poti do produkcijskega sistema.
\end{itemize}

Na konceptualni ravni naloga predstavi model grafa zaupanja, v katerem so semantični opisi akterjev, politik in dokazov povezani z zapisom odločitev na verigi blokov. Na arhitekturni ravni naloga združi ontologijo, preverljive poverilnice, decentralizirano omrežje orakljev in pametno pogodbo \texttt{TrustGraph.sol} v delujoč prototip. V implementaciji pokažemo, kako lahko trije oraklji z različnimi algoritmi izračunajo ocene zaupanja, jih pošljejo agregatorju in preko njega zapišejo agregirano odločitev na verigo blokov. Varnostna analiza na koncu prikaže ključne grožnje in obstoječe protiukrepe ter nakaže, kaj bi bilo potrebno nadgraditi pri prehodu v produkcijsko okolje.

Metodološko naloga združuje pregled literature s področja upravljanja zaupanja v razpršenih sistemih in farmacevtskih dobavnih verigah, načrtovanje semantičnega modela in arhitekture, implementacijo prototipa ter eksperimentalno vrednotenje na podlagi scenarijev. Scenariji služijo kot ponovljivi primeri, s katerimi pokažemo, kako se ocene zaupanja spreminjajo ob novih podatkih, na primer ob incidentih v transportu, ter kako se različni pogledi orakljev zlivajo v enotno, na verigi zapisano odločitev. Varnostni del naloge temelji na modelu STRIDE in analizira pametno pogodbo ter decentralizirano omrežje orakljev z vidika ključnih razredov groženj.

Struktura naloge je naslednja. V drugem poglavju predstavimo teoretično ozadje: modele zaupanja v razpršenih sistemih, semantične tehnologije, verige blokov, preverljive poverilnice in decentralizirana omrežja orakljev. V tretjem poglavju opišemo zasnovo sistema, vključno z ontologijo, arhitekturo in izbranimi algoritmi orakljev. Četrto poglavje je posvečeno implementaciji prototipa, opisu komponent ter treh orakljev in agregatorja. V petem poglavju predstavimo scenarije uporabe in rezultate izvajanja prototipa. V šestem poglavju izvedemo varnostno analizo pametne pogodbe in omrežja orakljev. V sedmem poglavju obravnavamo pot od prototipa do produkcijskega sistema ter operativne in regulativne vidike. Zaključek povzema glavne ugotovitve in prispevke naloge ter nakaže smeri nadaljnjega dela.
...
Ontologija vsebuje tudi posamezne primerke akterjev, na primer \textit{Pfizer}, \textit{DHL} in \textit{MediPlus}, ki imajo pripisane konkretne vrednosti lastnosti. Spodaj je prikazan primer zapisa prevoznika v RDF/Turtle obliki:
...
\begin{enumerate}
  ...
  \item \textbf{Orakelj zazna novo zahtevo in začne proces izračuna}\newline
    ...
    Če je orakelj nastavljen za tovrstne izračune, začne postopek ocenjevanja. Najprej pridobi vse potrebne podatke iz plasti znanja: semantične lastnosti akterja preko SPARQL poizvedb strežniku Fuseki ter pripadajoča preverljiva dokazila iz baze VC. Orakelj tako zagotovi, da bo izračun temeljil na najnovejših, verodostojnih in digitalno podpisanih podatkih.
  ...
  \item \textbf{Orakelj osveži podatke v plasti znanja}\newline
    ...
    \begin{itemize}
      \item semantične lastnosti akterja iz ontologije prek SPARQL poizvedb strežniku Fuseki,
  ...
  \item \textbf{Agregator sprejme poročila in izvede preverjanje}\newline
    ...
    \begin{quote}\texttt{recordOracleSubmission(requestId, evaluatorHash, report, credentialHash)}\end{quote}
...
\begin{enumerate}
  ...
  \item \textbf{Zajem podatkov iz plasti znanja}\newline
    ...
    \begin{itemize}
      \item ontološki podatki prek strežnika Fuseki (SPARQL poizvedbe ali lokalni OWL),
  ...
  \item \textbf{Izvedba lokalnega izračuna zaupanja}\newline
    ...
    \begin{itemize}
      ...
      \item \texttt{credential\_hash}, ki označuje VC dokazilo uporabljeno pri izračunu.
    \end{itemize}
  ...
  \item \textbf{Pošiljanje podpisanega poročila agregatorju}\newline
    ...
    Podpisano poročilo nato pošlje agregatorju prek zahteve HTTP (\texttt{POST /reports}), skupaj z identifikatorjem zahteve.
    Da agregator poročilo sprejme, mora orakelj poslati podatke v standardizirani obliki \texttt{SignedOracleReport}.  
    Ta vsebuje dva dela:
    \begin{itemize}
...
\end{itemize}
Če subjekt za vse zahtevane kriterije zadošča pogojem, deterministični del vrne odločitev \emph{PASS}, sicer \emph{FAIL}. Ta del je najbližje \emph{dokaznim modelom zaupanja}, kjer se odločitve sprejemajo na podlagi preverljivih dokazov in jasno določenih pragov.
...
\]
kjer je v implementaciji uporabljena lestvica $\textit{obs\_scale} = 9$. Za »popolno« izpolnjen pogoj ($s_0 \approx 1$) dobimo pričakovano vrednost Beta porazdelitve $\mathbb{E}[X] \approx 0{,}91$, kar predstavlja visoko, a ne absolutno zaupanje.
...
Za posamezen subjekt se verjetnostna ocena izračuna kot uteženo povprečje srednjih vrednosti Beta posteriorjev aktivnih kriterijev. Uteži so zapisane v polju \texttt{\_weights} v politiki in se normalizirajo tako, da zadošča:
\[
...
\begin{itemize}
  ...
  \item povzetek poverilnic (\texttt{credential\_hash}) ni v nasprotju z
        lokalno shrambo poverilnic (če ta obstaja).
\end{itemize}
...
Ko je agregacija uspešna, agregator za dani \texttt{request\_id} izvede klic
\texttt{fulfillTrustReport} v pametni pogodbi \texttt{TrustGraph.sol}. V
...
Čeprav je bila rešitev v tej nalogi konkretizirana na farmacevtski dobavni verigi, je osnovni vzorec precej širši. V jedru ne upravljamo zaupanja v »zdravila«, temveč v akterje, ki si izmenjujejo podatke in prevzemajo odgovornost v porazdeljenem ekosistemu: organizacije z različnimi vlogami, pogosto nasprotujočimi si interesi, heterogene podatkovne vire in potrebo po neodvisno preverljivem zapisu o tem, komu in pod kakšnimi pogoji zaupamo. Enak vzorec se pojavi v drugih dobavnih verigah: pri sledljivosti hrane, logistiki elektronskih komponent, oskrbi z energenti ali recikliranju materialov. Povsod, kjer so pomembni certifikati, revizije, operativna telemetrija in regulatorji, a primanjkuje enotnega, transparentnega grafa zaupanja, ki bi povezoval vse te signale.
...
V magistrski nalogi smo obravnavali problem upravljanja zaupanja v farmacevtski dobavni verigi, kjer sodeluje več med seboj nepopolnoma zaupanja vrednih akterjev, ki si izmenjujejo občutljive informacije in prevzemajo regulativno pomembne odločitve. Izhodišče je bilo, da obstoječi pristopi k upravljanju zaupanja v praksi pogosto temeljijo na zaprtih bazah podatkov, neformalnih pogodbenih razmerjih in ročnih revizijah, kar otežuje ponovljivost, transparentnost in strojno preverljivost odločitev. Cilj naloge je bil zato zasnovati in implementirati prototipni sistem, v katerem so semantični modeli in veriga blokov povezani v enoten mehanizem za izračun in objavo odločitev o zaupanju.

Najprej smo opravili pregled literature s področja upravljanja zaupanja v razpršenih sistemih ter identificirali ključne modele in njihovo uporabnost v farmacevtskem kontekstu. Na tej podlagi smo definirali konceptualni model, v katerem ontologija opisuje tipe akterjev in relevantne lastnosti. V nadaljevanju naloge je predstavljena arhitektura sistema, ki ta konceptualni model poveže z verigo blokov: zasnovali smo pametno pogodbo, ki hrani končne odločitve in prispevke orakljev, ter decentralizirano omrežje orakljev (DON), v katerem različni algoritmi izračuna zaupanja na osnovi ontologije, poverilnic in telemetrije pripravljajo podpisana poročila, agregator pa iz njih izpelje enotno odločitev.

V implementacijskem delu smo prototip udejanjili v testnem okolju s pomočjo vsebnikov Docker, testnega Ethereum omrežja in treh orakljev z različnimi pogledi na zaupanje. Razvili smo končen potek od sprožitve zahteve po poročilu, prek izračuna v orakljih in agregacije, do trajnega zapisa rezultata v pametno pogodbo in njegovega branja z vidika akterjev v dobavni verigi. Skozi podrobno analizirana scenarija smo pokazali, kako se različni pogledi na zaupanje odražajo v konkretnih ocenah in kako agregator iz njih izpelje odločitev, ki se dinamično prilagaja spremembam v telemetriji. Zadnji prispevek naloge je varnostna analiza dela sistema, ki teče na verigi blokov in v decentraliziranem omrežju orakljev. Z uporabo modela STRIDE smo sistematično identificirali glavne razrede groženj za pametno pogodbo in za omrežje orakljev, opisali obstoječe prototipne protiukrepe. Na koncu smo izpostavili ključne zahteve, ki bi jih bilo treba izpolniti pri prehodu v produkcijsko okolje in opisali, kako bi bil razvit sistem uporaben tudi v drugih sektorjih, ne le v farmaciji.

\section{Nadaljnje delo}
\label{subsec:omejitve-nadaljnje}

Predstavljeni sistem je zasnovan kot prototip na testnem omrežju z omejenim številom orakljev pod enotnim upravljanjem in s simuliranimi podatkovnimi viri. Ne naslavlja celotne kompleksnosti realnega farmacevtskega ekosistema, kjer bi oraklje upravljali različni akterji, podatki bi prihajali iz produkcijskih informacijskih sistemov, pametna pogodba pa bi tekla v okolju z dejanskimi operativnimi, varnostnimi in regulativnimi zahtevami. Trenutna implementacija le nakazuje uporabo preverljivih poverilnic in osnovnih vlog v pametni pogodbi.

Nadaljnje delo mora zato zajeti razširitev sistema v smeri realističnega okolja: vzpostaviti je potrebno decentralizirano omrežje orakljev z oraklji, ki jih dejansko upravljajo različni akterji, sistem prenesti na javno ali zasebno verigo z jasno definiranimi vlogami, vključiti infrastrukturo za izdajo in preverjanje poverilnic (npr.\ EBSI), definirati mehanizme spodbujanja in kaznovanja orakljev, okrepiti varnostne mehanizme  ter sistem preveriti na širšem naboru scenarijev in podatkov. V širšem smislu bi lahko raziskali tudi prenos istega vzorca grafa zaupanja na druge dobavne verige in domene (npr.\ logistika, pametna mesta), kjer več neodvisnih akterjev potrebuje skupno, razložljivo in kriptografsko preverljivo plast zaupanja.


\appendix
\chapter{OWL ontologija dobavne verige}
\label{app:trust-ontology}
\lstinputlisting[
  language=XML,
  caption={OWL ontologija za model zaupanja v farmacevtski verigi},
  breaklines=true,
