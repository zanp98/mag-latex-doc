...
V družboslovnem kontekstu je zaupanje pogosto opredeljeno kot stanje pozitivnih pričakovanj glede dejanj druge osebe v okoliščinah, kjer obstaja določena stopnja tveganja \cite{boon1991interpersonal}. Gambetta \cite{gambetta1988trust} zaupanje definira kot subjektivno verjetnost, da bo akter opravil določeno dejanje, še preden je to mogoče preveriti. Mayer, Davis in Schoorman \cite{mayer1995integrative} pa ga opredelijo kot pripravljenost ene stranke, da se izpostavi ranljivosti glede na dejanja druge stranke, ob pričakovanju, da bo ta delovala v skladu s pričakovanji.
...
Za odločanje, ali je akter zaupanja vreden, avtorja uvedeta pragova $c_1$ in $c_2$. Prag $c_1$ določa minimalno število skupnih interakcij, ki jih mora imeti akter, da lahko opazovalec sprejme informirano odločitev. Prag $c_2$ pa izraža, kako visoko vrednost zaupanja je opazovalec pripravljen sprejeti kot še dovolj dobro: bolj toleranten akter lahko izbere nižji $c_2$, manj toleranten pa višjega. Akter $u$ se obravnava kot zaupanja vreden, če hkrati velja $I(u,t) > c_1$ in $T(u,t) > c_2$ \cite{xiong2002building}.
...
Osrednji poudarek naloge je na zasnovi in implementaciji sistema, ki omogoča, da različni akterji izvedejo izračun zaupanja po lastnih pravilih in z uporabo lastnih algoritmov. Sistem zato ne vsiljuje enotnega modela, temveč zagotavlja skupen mehanizem za pridobivanje vhodnih podatkov ter za preverljiv in trajen zapis rezultatov v verigo blokov, neodvisno od izbrane metode izračuna.
...
Primer takšne zasnove je sistem ASTRAEA, predlagan s strani Adlerja in sodelavcev, ki uporabi glasovalno igro z vložki in ločene vloge udeležencev (oddajatelji trditev, glasovalci, certificiranci), da z ekonomsko motivacijo spodbudi udeležence k poštenemu poročanju o resničnosti trditev \cite{adler2018astraea}.
Podobno pristopajo tudi druge rešitve, kjer se omrežja orakljev, kot je Chainlink, uporabljajo za zbiranje podatkov iz več API-jev in senzorjev ter za izračun dodatnih meril (npr. reputacije) še pred zapisom v verigo blokov. Tak pristop uporabijo Kochovski in sodelavci v članku \textit{Trust management in a blockchain based fog computing platform with trustless smart oracles} \cite{kochovski2019trust}.
...
Pri razvoju sistema smo uporabili različna orodja in platforme, ki skupaj omogočajo obdelavo semantičnih podatkov, izvajanje izračuna zaupanja ter zapis rezultatov v verigo blokov. V tem razdelku podajamo pregled najpomembnejših tehnologij in njihovih vlog v sistemu.
...
Poleg podatkov iz ontologije sistem uporablja še preverljive poverilnice (VC), ki jih izdajajo regulatorji ali druge zaupanja vredne organizacije. Te vsebujejo trditve o akterjih, kot so veljavnost licence, GMP skladnost ali datum zadnje presoje kakovosti. Zapisane so v formatu JSON-LD in digitalno podpisane.
...
Drugi sklop groženj se nanaša na integriteto stanja pametne pogodbe (angl.\ tampering). Čeprav veriga blokov zagotavlja nespremenljivost zgodovine transakcij, to samo po sebi ne prepreči, da pametna pogodba zaradi neustrezno preverjenih parametrov ali pomanjkljivih logičnih pogojev vnese v svoje stanje nekonsistentne podatke. Do takšnih razmer lahko pride denimo takrat, ko se nov rezultat napačno poveže z drugim subjektom ali ko nova odločitev dobi časovni žig, ki je kronološko starejši od časovnega žiga že obstoječe odločitve za istega subjekta. V tem primeru “računanje časovnega žiga v preteklost” pomeni, da se nova odločitev zabeleži prej kot njej predhodna, saj je njen pripisani časovni žig manjši (starejši). V našem primeru časovni žig nastopa kot kriptografsko zaščiten podatek o času, ki se vključi v zgostitev in podpis poročila ter se zapiše v verigo blokov skupaj z ostalimi parametri. Takšno stanje je semantično nelogično.
