...
\paragraph{$Dokaz.$} Dokažimo, da velja $T(u,t) \in [0,1]$.\\
V skladu z definicijo funkcij privzemimo, da za vse $v \in P, v \neq u$ in dani $t$ velja
$0 \leq S(u,v,t) \leq I(u,v,t)$ ter $0 \leq \mbox{Cr}(v,t) \leq 1$, poleg tega pa ima akter $u$ vsaj eno interakcijo, torej velja $\sum_{v \in P, v \neq u} I(u,v,t) > 0$.
\\
Ker je $S(u,v,t) \ge 0$ in $\mbox{Cr}(v,t) \in [0,1]$, sledi
$0 \leq S(u,v,t)\,\mbox{Cr}(v,t) \leq S(u,v,t)$.
Zaradi $S(u,v,t) \leq I(u,v,t)$ dobimo
$0 \leq S(u,v,t)\,\mbox{Cr}(v,t) \leq I(u,v,t)$
za vse $v \in P, v \neq u$.
Če to neenakost seštejemo po vseh takih $v$, dobimo
\[
  0 \leq \sum_{v \in P, v \neq u} S(u,v,t)\,\mbox{Cr}(v,t)
      \leq \sum_{v \in P, v \neq u} I(u,v,t).
\]
\\
Ker je imenovalec strogo pozitiven, velja
\[
  0 \leq
  T(u,t)
  = \frac{\sum_{v \in P, v \neq u} S(u,v,t)\,\mbox{Cr}(v,t)}
...
Primer takpšnega modela predstavijo Lin in sodelavci v članku \textit{A Reputation and Trust Management Broker Framework for Web Applications} \cite{lin2005reputation}. 
...
Tehnologija veriženja blokov je bila prvič predstavljena leta 2008 v sklopu kriptovalute Bitcoin \cite{nakamoto2008bitcoin}. Osnovna ideja je zasnova podatkovne strukture, ki povezuje zapise v verigo blokov. Vsak blok vsebuje podatke, zgoščeno vrednost prejšnjega bloka in časovni žig. Takšna vezava med bloki preprečuje naknadne spremembe zapisanih podatkov, saj bi vsaka sprememba v enem izmed starejših blokov spremenila njegovo zgoščeno vrednost. Posledično pa se zgoščena vrednost ne bi več ujemala z vrednostjo, shranjeno v naslednjem bloku \cite{crosby2016blockchain}.
...
Proces podpisovanja se začne z ustvarjanjem zgoščene vrednosti sporočila. Podpisnik nato uporabi svoj zasebni ključ, ki ga algoritem s pomočjo operacij na eliptični krivulji pretvori v digitalni podpis. Rezultat tega postopka je podpis, sestavljen iz trojke $(v, r, s)$, ki tvori končni podpis. Funkcija \texttt{sign\_message} v našem sistemu sprejme zgoščeno vrednost sporočila in zasebni ključ podpisnika ter vrne podpis, vrednosti $(v, r, s)$ in podpisano sporočilo.

Podpis preverjamo s funkcijo \texttt{recover\_message}. Ta na podlagi podpisa in zgoščene vrednosti sporočila obnovi naslov podpisnika. Če se izračunani naslov ujema z znanim naslovom pošiljatelja, je pristnost sporočila potrjena.
...
\begin{enumerate}
  ...
  \item \textbf{Pošiljanje podpisanega poročila agregatorju}\newline
    ...
    \begin{itemize}
      \item \texttt{subject} – 32-bajtni identifikator subjekta oz. zgoščena vrednost DID,
...
\begin{itemize}
  ...
  \item \textbf{Zastavice} Oraklja A in C nastavljata zastavico
        ...
        ravni.
  \item \textbf{Politike in poverilnice} Vsa tri poročila uporabljajo enako zgoščeno vrednost poverilnic \texttt{credential\_hash}, kar pomeni, da se še vedno sklicujejo na isto veljavno preverljivo poverilnico. Agregator pri tem preveri, da poverilnica ni označena kot preklicana. To pomeni, da je sprememba ocene iz \texttt{PASS} (scenarij 1) v \texttt{FAIL} (scenarij 2) posledica izključno poslabšanja operativnih meril in zunanjih podatkov, ne pa spremembe formalnih dokazil.
\end{itemize}
...
Drugi sklop groženj se nanaša na integriteto stanja pametne pogodbe (angl.\ tampering). Čeprav veriga blokov zagotavlja nespremenljivost zgodovine transakcij, to samo po sebi ne prepreči, da pametna pogodba zaradi neustrezno preverjenih parametrov ali pomanjkljivih logičnih pogojev vnese v svoje stanje nekonsistentne podatke. Do takšnih razmer lahko pride denimo takrat, ko se nov rezultat napačno poveže z drugim subjektom ali ko nova odločitev dobi časovni žig, ki je kronološko starejši od časovnega žiga že obstoječe odločitve za istega subjekta. V tem primeru “računanje časovnega žiga v preteklost” pomeni, da se nova odločitev zabeleži prej kot njej predhodna, saj je njen pripisani časovni žig manjši (starejši). V našem primeru časovni žig nastopa kot kriptografsko zaščiten podatek o času, ki se vključi v zgoščeno vrednost in podpis poročila ter se zapiše v verigo blokov skupaj z ostalimi parametri. Takšno stanje je semantično nelogično.
...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{|X|X|X|}
    ...
    Spreminjanje vsebine poročil med prenosom ali v agregatorju 
      & Podpis predstavitve JSON, preverjanje zgoščenih vrednosti in zavrnitev pri neskladju
      & Enotski testi serializacije ter preverjanja podpisov in zgoščenih vrednosti \\
    \hline
    Orakelj zanika, da je oddal določeno poročilo 
      & Shranjevanje zgoščenih vrednosti podpisanih poročil in obnova naslova iz podpisa 
    ...
    Prekomerno evidentiranje identitet subjektov ali poverilnic 
      & Zgoščene vrednosti identifikatorjev v dnevnikih 
...
Pri napadih s spreminjanjem podatkov ščitimo predvsem integriteto vsebine poročil. Ker je podpisan JSON celotnega poročila, agregator zazna vsako spremembo vsebine med prenosom ali vmesnim shranjevanjem. Za produkcijsko okolje je smiselno dodati revizijsko sled (na primer zapis zgoščenih vrednosti sprejetih poročil), kar omogoča naknadno preverjanje skladnosti med prejetimi poročili in zapisi na verigi. Enotski testi pri tem preverijo, da serializacija poročila v kanonično obliko vedno vrne enak rezultat in da vsaka sprememba vsebine povzroči neuspeh preverjanja podpisa.
