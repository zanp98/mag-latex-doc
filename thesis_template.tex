...
Abdul-Rahman in Hail v svojem modelu zaupanja \cite{abdul1998distributed} predlagata, da se zaupanje v ciljno entiteto $T$ izračuna kot produkt vmesnih ocen na poti od začetne entitete do ciljne. Naj bo $n$ število entitet na poti do ciljne entitete, $rtv(i) \in \{1,2,3,4\}$ vrednost priporočila za $i$-tega posrednika (preračunana iz subjektivnih ocen), in $tv(T) \in [0,1]$ neposredna vrednost zaupanja v ciljno entiteto.

Skupna vrednost zaupanja $tv_T$ se nato izračuna kot:

\begin{equation}
  tv_T = \left( \prod_{i=1}^{n} \frac{rtv(i)}{4} \right) \cdot tv(T)
  \label{eq:abdul-recommendation}
\end{equation}

Kjer je vsak $rtv(i)$ deljen s 4 zato, da se normalizira na interval $[0.25, 1.0]$.
Če obstaja več neodvisnih poti med entitetama, se končna vrednost zaupanja določi kot aritmetična sredina rezultatov posameznih poti.

Priporočilni modeli gradijo zaupanje na osnovi izkušenj in posredovanih ocen drugih entitet.
Vsaka entiteta ocenjuje druge glede na pretekle interakcije, te ocene pa se lahko delijo naprej kot priporočila.
Abdul-Rahman in Hailes predlagata model, ki uporablja pogojno tranzitivnost zaupanja – entiteta A lahko zaupa entiteti C, če A zaupa B kot priporočitelju in lahko ovrednoti njegovo oceno.
...
\end{equation}
kjer $rtv(i)$ predstavlja stopnjo zaupanja v posameznega posrednika, $tv(T)$ pa končno vrednost zaupanja v ciljno entiteto \cite{abdul1998distributed}.
...
Sistem lahko določi pragove $C_1$ in $C_2$, pri čemer entiteta velja za zaupanja vredno, če velja $I(u,t) > C_1$ in $T(u,t) > C_2$ \cite{xiong2002building}.

Takšni sistemi so učinkoviti pri akumuliranju ocen, vendar občutljivi za napačne ali zlonamerne povratne informacije.
Zato številne rešitve uvajajo uteževanje priporočil glede na zanesljivost virov ali na starost podatkov.
...
\end{equation}
kjer $T_{\text{old}}$ predstavlja prejšnjo vrednost zaupanja, $r$ novo oceno po transakciji, $N$ število izvedenih interakcij, $\beta$ faktor dušenja, $\Delta t$ pa čas od zadnje posodobitve \cite{lin2005reputation}.
Na ta način sistem zagotavlja, da nove izkušnje bolj vplivajo na oceno, stare pa postopoma izgubijo težo.
...
Verjetnostni del iste kriterije pretvori v številsko verjetnostno oceno v intervalu $[0,1]$, ki jo orakelj kasneje pretvori v osnovne točke (angl.\ \emph{basis points}) med 0 in 10000. Za vsak kriterij se vodi aposterior prazdelitve beta $B(\alpha, \beta)$, ki modelira zaupanje v izpolnjevanje tega kriterija.
