...
Predlagani sistem temelji na kombinaciji semantičnega modeliranja, decentralizirane infrastrukture in preverljivih dokazil. Akterji v dobavni verigi so formalno opisani z uporabo ontologije, ki omogoča predstavitev njihovih lastnosti. Zaupanje se ocenjuje na podlagi več neodvisnih virov z uporabo orakljev, ki izračune izvajajo zunaj verige blokov, končni rezultati pa se zapisujejo v verigo blokov.

V nadaljevanju naloge najprej predstavimo teoretično ozadje upravljanja zaupanja v razpršenih sistemih ter obstoječe pristope, uporabljene v farmacevtskih dobavnih verigah. Nato opišemo konceptualni model zaupanja in arhitekturo predlaganega sistema, vključno z vlogo ontologije, orakljev in verige blokov. Sledi poglavje implementacije, kjer podrobno predstavimo zasnovo in delovanje prototipa. V poglavju evalvacije prikažemo eksperimentalne scenarije, ki demonstrirajo praktično delovanje sistema, ter analiziramo njegove varnostne lastnosti z uporabo modela groženj. V zaključku povzamemo ključne ugotovitve in predlagamo možnosti za nadaljnji razvoj rešitve.
...
Predstavljeni modeli in enačbe v tem poglavju služijo kot pregled možnih pristopov k izračunu zaupanja v razpršenih okoljih. V okviru te naloge jih obravnavamo kot primere, ki jih je mogoče uporabiti pri implementaciji izračuna v orakljih, vendar njihova uporaba ni predpisana.
...
Plast znanja združuje semantične opise akterjev in njihove lastnosti z regulatorno potrjenimi podatki iz preverljivih dokazil. Oraklji pri izračunu zaupanja dostopajo do obeh virov: iz Fusekija pridobijo opisna merila, iz VC-jev pa dodatne podatke, ki jih ontologija ne vsebuje. S tem plast znanja omogoča enoten in strukturiran nabor podatkov, na katerem temelji celoten izračun zaupanja.
...
\begin{enumerate}
  ...
  \item \textbf{Agregator sprejme poročila in izvede preverjanje}\newline
    ...
    \begin{itemize}
      ...
      \item ali se zgostitve pravil in dokazil ujemajo z lokalnimi podatki.
    \end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Preverjanje podpisov in veljavnosti poročil}\newline
    ...
    \begin{itemize}
      ...
      \item ali se zgostitve pravil in dokazil ujemajo z njegovimi lokalnimi podatki.
    \end{itemize}
...
\begin{itemize}
  ...
  \item čas izračuna,
  \item zgostitve pravil in dokazil,
...
\end{itemize}
Če subjekt za vse zahtevane kriterije zadošča pogojem, deterministični del vrne odločitev \texttt{PASS}, sicer \texttt{FAIL}. Ta del je najboljši približek \emph{dokaznim modelom zaupanja}, kjer se odločitve sprejemajo na podlagi preverljivih dokazov in jasno določenih pragov.
...
kako semantični ter telemetrijski signali lahko pretehtajo preverljiva dokazila
...
Na verigo se nikoli ne pošiljajo surovi vhodni podatki. Zapisujejo se le zgostitve (povzetki politike, povzetki preverljivih dokazil in podatki iz ontologije), agregirana ocena ter opozorilne zastavice. Pametna pogodba tako deluje kot kriptografsko zavarovana osrednja plast, ki zagotavlja integriteto rezultatov, ne da bi hranila osebne ali poslovno občutljive podatke.
